...
parser.add_argument('--data_path', default='C:/Users/USSTz/Desktop/clothing1m/images', type=str, help='path to dataset')
...
parser.add_argument('--resume', default=True, type=bool, help = 'Resume from the warmup checkpoint')
...
def val(net,val_loader,k):
    ...
    print("\n| Validation\t Net%d  Acc: %.2f%%, best_aac: %.2f" %(k,acc, best_acc[k-1]))
    if acc > best_acc[k-1]:
        best_acc[k-1] = acc
...
if __name__ == '__main__':
    ...
    loader = dataloader.clothing_dataloader(root=args.data_path, batch_size=args.batch_size,
                                            warmup_batch_size = args.batch_size*2, num_workers=0,
    ...
    for epoch in range(start_e, args.num_epochs+1):
        ...
        if epoch<warm_up:
            ...
            if threshold.item()>args.d_u:
                threshold = threshold - (threshold-torch.min(prob2))/args.tau
            ...
            if threshold.item()>args.d_u:
                threshold = threshold - (threshold-torch.min(prob1))/args.tau
        ...
        print('\n| Epoch:%d \t  before_val_Acc: %.2f%% (%.2f%%) \n' % (epoch, accs[0], accs[1]))
        log.write('Epoch:%d \t  before_val_Acc: %.2f%% (%.2f%%) \n' % (epoch, accs[0], accs[1]))
        log.flush()
