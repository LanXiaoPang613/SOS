...
class cifar_dataset(Dataset): 
    def __init__(self, dataset, sample_ratio, r, noise_mode, root_dir, transform, mode, noise_file='', pred=[], probability=[], log=''): 
        ...
        if self.mode=='test':
            ...
            self.clean_labels = np.array(train_label).astype(np.int64)
            ...
            if os.path.exists(noise_file):             
                ...
                for kk in range(num_class):
                    self.class_ind[kk] = [i for i, x in enumerate(noise_label) if x==kk]
            self.noise_labels = np.array(noise_label).astype(np.int64)
            if self.mode == 'all':
                ...
                if self.mode == "labeled":
                    ...
                    self.probability = [1-probability[i] for i in pred_idx]     # high prob 意味着low prediction scores consistence
...
class cifar_dataloader():  
    ...
    def run(self, sample_ratio, mode, pred=[], prob=[]):
        if mode=='warmup':
            ...
            unlabeled_dataset = cifar_dataset(dataset=self.dataset, sample_ratio= sample_ratio, noise_mode=self.noise_mode,
                                              r=self.r, root_dir=self.root_dir, transform=self.transforms["unlabeled"],
            ...
            test_dataset = cifar_dataset(dataset=self.dataset, sample_ratio= sample_ratio, noise_mode=self.noise_mode,
                                         r=self.r, root_dir=self.root_dir, transform=self.transform_test, mode='test')
